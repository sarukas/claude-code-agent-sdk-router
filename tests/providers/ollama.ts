// Ollama provider tests — local OpenAI-compatible inference.
// Tool use may not be supported depending on the model.
...
// Many Ollama models don't support tool calling
const TOOL_USE_EXPECTED_FAIL = ['ollama'];
...
export const tests: TestFn[] = [
  ...
  {
    name: 'tool_use',
    expectedFailProviders: TOOL_USE_EXPECTED_FAIL,
    fn: async (ctx) => {
      const res = await sendMessage(ctx, {
  ...
  },
  {
    ...
    fn: async (ctx) => {
      ...
      // Note: most Ollama models don't support tool calling, so this is EXPECTED_FAIL
