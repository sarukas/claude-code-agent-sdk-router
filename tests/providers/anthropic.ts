// Anthropic provider tests — native pass-through, all features expected to work.
...
export const tests: TestFn[] = [
  {
    ...
    fn: async (ctx) => {
      ...
      assert(res.role === 'assistant', 'role is assistant');
    },
  },
  {
    name: 'tool_use',
    fn: async (ctx) => {
      ...
      assert(toolBlocks[0].input && typeof toolBlocks[0].input === 'object', 'tool has input object');
    },
  },
  {
    name: 'streaming',
  ...
  },
  {
    ...
    fn: async (ctx) => {
      ...
      // Turn 2: send simulated search results back, verify model summarizes
      ...
      // Check that the model actually used the search results
